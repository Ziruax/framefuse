...
// ChromaSection module (used by BOTH the media panel expander and the
// settings panel's dedicated Chroma tab).
import { ChromaSection, seekVideoTo } from "@/components/ChromaSection";
import {
  SFX_LIBRARY,
  getSfxDef,
  renderSfxBuffer,
...
interface MediaPanelProps {
  ...
  /** v1: dedicated video picker — the Images button now filters images ONLY
...
export function MediaPanelBase({
  ...
  // v1.14: Disclaimer / intro lead-in card — the FIRST thing the video
  // shows. Any filename (naming rules never apply); images hold for the
  // picked duration, videos play their length. Hoisted into a const so it
  // renders in BOTH the empty state (pick it before any media exists) and
  // the populated library (list AND grid — it sits outside the view
  // switch, above the audio-track chip).
    ...
    >
      ...
      >
        ...
        <button
          ...
          title="Attach an audio track — MP3/WAV/M4A/OGG (voiceover / music)"
        >
      ...
      <div className="ff-scroll flex-1 overflow-y-auto">
        ...
        {segments.length === 0 && mediaTab !== "audio" && mediaTab !== "subs" && (
          <div className="p-4">
            ...
            {disclaimerSection && (
              <div className="mt-3">{disclaimerSection}</div>
            )}
        ...
        {segments.length > 0 && (
          <div className="space-y-1.5 p-3">
            ...
            {/* v1.14: Disclaimer / intro lead-in card — hoisted into
                disclaimerSection so it renders in BOTH the empty state (0
 ...
 *  v1: the canonical FrameSource + seekVideoTo/loadImageElement/drawSnapshot
 *  helpers moved to src/components/ChromaSection.tsx (shared with the
 *  settings-panel Chroma tab); MediaPanel imports seekVideoTo from there. */
 ...
 *  v1: starts COLLAPSED (was open — pushed the media list far down and
 *  buried the uploads under a wall of SFX chips). */
