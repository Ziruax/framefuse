...
def main():
    ...
    for s in SIZES:
        ...
        if s >= 256:
            # 256px rides PNG (spec-sanctioned, huge DIB savings).
            buf = io.BytesIO()
            im_s.save(buf, "PNG", optimize=True)
            entries.append((buf.getvalue(), (s, s)))
        else:
            # ≤128px rides DIB — the Windows-universal format (v7 FIX B:
            # all-PNG small entries showed as the blank "white paper" icon).
            entries.append((dib_bytes(im_s), (s, s)))
    make_ico(entries, "build/icon.ico")
    print(f"build/icon.ico written ({len(SIZES)} entries: DIB<=128 + PNG256)")
